...
## 1.5.4 (2026-08-15)
- Event Bus 主路径读取标准事件类型，裸直通回复改走 MessageOps 并保留回复锚点。
...
- 补充真实 Event Bus 载荷形态的回归测试，避免测试中的理想化 `is_bot` 字段再次掩盖问题。

## 1.5.1 (2026-07-11)
- 随机回复概率 schema 在字符串输入之外兼容旧版数字配置，避免升级后保存其它账号配置时被平台完整校验阻断。
- 运行时解析和新配置输入行为保持不变，后续保存会继续使用字符串形式。
- 使用说明明确账号级二次开关只控制低延时裸直通，关闭后标准 Event Bus 入口仍可继续运行。
