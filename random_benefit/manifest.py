...
CONFIG_SCHEMA = {
    "type": "object",
    "x-ui-mode": "single",
    "x-usage-guide": "从当前账号的已允许会话中选择要开启随机福利的群组。选中群组默认纳入作用范围；管理员可在群内发送独立配置的开启指令、暂停指令和状态指令控制当前群组。插件只会从普通群友发言中随机命中，会排除当前账号自己的消息和 Bot 账号消息。为防刷屏，命中回复后会进入全群冷却和同一用户冷却，冷却秒数设为 0 可关闭。裸直通需要安装后在账号配置里手动开启 direct_passthrough.enabled=true。",
