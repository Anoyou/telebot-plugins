...
INVALID_PRIZE_MESSAGE_TEMPLATE_DEFAULT = "请指定奖励金额，例如：{prefix}{command} {example}"
LEGACY_INVALID_PRIZE_MESSAGE_TEMPLATE_DEFAULT = "请指定奖励金额，例如：,{command} {example}"
...
CONFIG_SCHEMA = {
    ...
    "properties": {
        ...
        "round_message_template": {
            "type": "string",
            "title": "开局消息模板",
            "description": "支持占位符：{target_sum}、{prize}、{timeout}、{guess_cooldown}、{command}、{prefix}。旧配置中的 {title}、{target_line}、{guide_line}、{reward_line} 仍会兼容展开，但新模板建议直接写完整文案。",
