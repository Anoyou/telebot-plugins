...
## 1.0.9 (2026-08-15)
- 将消息和文件发送迁移到 MessageOps，图片生成与编辑请求改走 ctx.http，移除 curl 回退并限制 OpenAI 域名。


## 1.0.7 (2026-07-14)
- 首次启动时若新目录尚无配置，原子复制源码目录中的旧 GI2 配置到 `ctx.data_dir` 账号目录；不覆盖新文件且保留旧文件。
