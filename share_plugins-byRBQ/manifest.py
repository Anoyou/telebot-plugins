...
CONFIG_SCHEMA = {
    "type": "object",
    "x-ui-mode": "single",
    "x-usage-guide": '发送 {prefix}{command} 按原 Pagermaid 方式查看插件列表并按序号分享插件文件；可在配置页调整触发指令名和启用状态。',
...
USAGE = ('发送 {prefix}{command} 按原 Pagermaid 方式查看插件列表并按序号分享插件文件；可在配置页调整触发指令名和启用状态。事件订阅：账号主人或授权管理员通过 userbot '
