...
## 1.1.35 (2026-08-15)
- 将总结推送、命令编辑和删除统一改为平台 MessageOps，AI 仍通过 ctx.ai 路由。
...
## 1.1.33 (2026-07-14)
- 按当前插件持久化规则，将总结任务与运行时配置文件改为写入 `ctx.data_dir`，首次启动自动迁移插件目录中的旧配置；未提供数据目录时安全降级为不落盘。
- 修正运行时版本常量滞后于 `plugin.json` 与 `manifest.py` 的问题，三处版本统一为 `1.1.33`。
- 最低 TelePilot 版本提升至 0.59.1，确保安装环境提供更新安全的插件数据目录。
