...
## 1.0.17 (2026-08-15)
- 将 UserBot 命令回复、退款和开奖动作统一收口到 MessageOps；退款先经平台 payout 成功后再修改注单状态，失败时保持原状态。

## 1.0.16 (2026-08-15)
- 新增标准 Event Bus 主入口；自动开奖通知与奖金派发统一改走 MessageOps，移除文本发奖兜底。
...
- 保持原有彩票下注、开奖、派彩金额与文案逻辑完全不变。

## 1.0.12 (2026-07-04)
- 移除旧 `result_contract.send_via` 样板，普通回复改为继承当前会话通道。
- 自动开奖派彩优先通过 `ctx.messages.apply` 投递 `payout` action，由 userbot 执行。
