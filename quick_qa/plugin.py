...
def _session_sync_action(game: QuickQAGame, payload: dict[str, Any]) -> dict[str, Any]:
    participant_ids = sorted(int(user_id) for user_id in game.players if int(user_id) > 0)
    return {
        "type": "start_session",
        "chat_id": game.chat_id,
        "entry_key": ENTRY_KEY,
        "event_type": _event_type(payload) or "callback_query",
        "started_by_user_id": game.host_user_id or (participant_ids[0] if participant_ids else None),
        "started_by_message_id": _message_id(payload),
...
def _correct_answer_signature(question: QAQuestion) -> str:
    if question.answer_index < 0 or question.answer_index >= len(question.options):
        return ""
    return _text_signature(question.options[question.answer_index])


def _questions_look_duplicate(left: QAQuestion, right: QAQuestion) -> bool:
    left_question = _question_signature(left)
    right_question = _question_signature(right)
    ...
    if ratio >= 0.88:
        return True
    left_answer = _correct_answer_signature(left)
    right_answer = _correct_answer_signature(right)
    if left_answer and left_answer == right_answer and ratio >= 0.72:
        return True
    left_options = {_text_signature(option) for option in left.options if _text_signature(option)}
    right_options = {_text_signature(option) for option in right.options if _text_signature(option)}
    return len(left_options & right_options) >= 2 and ratio >= 0.68


def _dedupe_questions(questions: list[QAQuestion], limit: int = 0) -> list[QAQuestion]:
    unique: list[QAQuestion] = []
    for question in questions:
        if any(_questions_look_duplicate(existing, question) for existing in unique):
            continue
...
def _randomized_question_options(question: QAQuestion) -> QAQuestion:
    indexed_options = list(enumerate(question.options))
    random.shuffle(indexed_options)
    options = [option for _, option in indexed_options]
    answer_index = next(
        (idx for idx, (original_idx, _) in enumerate(indexed_options) if original_idx == question.answer_index),
...
def _merge_questions(existing: list[QAQuestion], incoming: list[QAQuestion], limit: int = 0) -> list[QAQuestion]:
    return _dedupe_questions([*existing, *incoming], limit=limit)
...
class QuickQAPlugin(Plugin):
    ...
    async def _handle_callback(self, ctx: PluginContext, payload: dict[str, Any], chat_id: int) -> list[dict[str, Any]]:
        ...
        async with self._lock(chat_id):
            ...
            if action == "join":
                return await self._join_free_game_from_callback_locked(ctx, payload, game)
            if action == "start":
    ...
    async def _join_free_game_from_callback_locked(
        self,
        ctx: PluginContext,
        payload: dict[str, Any],
        game: QuickQAGame,
        ...
        if game.entry_fee > 0:
            return [_answer_action(payload, "本局需要通过转账报名。", show_alert=True)]
        if game.phase != "lobby":
            return [_answer_action(payload, "报名已经结束。", show_alert=True)]
        user_id, name = _actor_id_name(payload)
        if not user_id:
            return [_answer_action(payload, "没有识别到你的 Telegram 身份。", show_alert=True)]
        if user_id in game.players:
            return [
                _answer_action(payload, "你已经加入本局。"),
                _session_sync_action(game, payload),
            ]
        if len(game.players) >= game.max_players:
            return [_answer_action(payload, "本局人数已满。", show_alert=True)]
        lobby_message_id = _message_id(payload)
        if lobby_message_id:
            game.lobby_message_id = lobby_message_id
            self._track_message_id(game, lobby_message_id)
        actions = await self._join_game_locked(
            ctx,
            game,
            user_id,
            name,
        ...
        return [_answer_action(payload, "加入成功"), *actions]
    ...
    async def _handle_payment(self, ctx: PluginContext, payload: dict[str, Any], chat_id: int) -> list[dict[str, Any]]:
        ...
        async with self._lock(chat_id):
            ...
            return await self._join_game_locked(
                ctx,
                game,
                user_id,
                name,
    ...
    async def _join_game_locked(
        ...
        *,
        ...
        if game.phase == "lobby":
            return [
                _session_sync_action(game, payload),
                await self._lobby_action(ctx, game, reply_to_message_id=message_id),
            ]
        return [
            _session_sync_action(game, payload),
            self._send_game_action(
    ...
    async def _start_questions_locked(
        ...
        questions = _dedupe_questions([q for kb in selected for q in kb.questions])
        if not questions:
            return [_answer_action(payload, "选中的题库没有可用题目", show_alert=True)]
        random.shuffle(questions)
        game.question_pool = [_randomized_question_options(question) for question in questions[: game.max_questions]]
    ...
    async def _send_payouts(self, ctx: PluginContext, game: QuickQAGame, settlements: list[SettlementItem]) -> None:
        payable = [item for item in settlements if item.amount > 0]
        if not payable:
        ...
        for index, item in enumerate(payable):
            ...
            payout_action: dict[str, Any] = {
                ...
                "reply_anchor_missing_text": "未找到玩家（{user_id}）的近期消息，本次奖励需要人工补发。",
            }
            if item.join_message_id:
                payout_action["reply_to_message_id"] = item.join_message_id
            await self._emit_actions(ctx, [payout_action])
            await _progress_log(
                ctx,
                "info",
    ...
    async def _selection_timeout(self, ctx: PluginContext, chat_id: int, game_id: str, seconds: int) -> None:
        ...
        async with self._lock(chat_id):
            ...
            kbs = self._available_kbs(ctx)
            game.selected_kb_ids = set()
            game.question_pool = _dedupe_questions([q for kb in kbs for q in kb.questions])
    ...
    async def _generate_kb_draft(
        ...
        *,
        ...
        valid_questions = [_randomized_question_options(q) for q in questions if q is not None]
    ...
    def _lobby_markup(self, game: QuickQAGame) -> dict[str, Any]:
        ...
        if game.entry_fee <= 0:
            rows.append([{"text": "加入游戏", "callback_data": f"{CALLBACK_PREFIX}:join:{game.game_id}"}])
        rows.append([{"text": "开始选择题库", "callback_data": f"{CALLBACK_PREFIX}:start:{game.game_id}"}])
    ...
    def _render_finish(
        ...
        *,
        ...
        for item in settlements:
            lines.append(f"- {_html(item.name)}：{item.points} 分 → {item.amount}")
        lines.extend([
            "",
    ...
    def _usage(self, ctx: PluginContext) -> str:
        ...
        return (
            ...
            f"{_code(prefix + self._command + ' 0')} 创建免费大厅，玩家点击“加入游戏”参与\n"
